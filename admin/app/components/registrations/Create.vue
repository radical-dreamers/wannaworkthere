<template lang="html">
  <div class="hero">
    <h1 class="title is-2">{{ $t('registrations.create.title' )}}</h1>
...
export default {
  ...
  methods: {
    ...
    create () {
      api.registrations.create(this.instance).then((result)=> {
        this.addMessage({
    ...
    update () {
      api.registrations.update(this.instance._id, this.instance).then((result)=> {
        this.addMessage({
